...
class CaesareNumberBruteForcer(GenericBruteForcer):
    def __init__(self, cipher_text, alphabet=Alphabet().ascii_albhabet):
        """
    ...
    def brute_force(self, format="ascii_charset", language="english", frequency_required=0.25):
        """
        ...
        >>> from cryptanalib.encoding.alphabet import Alphabet
        >>> cnbf = CaesareNumberBruteForcer([ord(number) for number in list("A5&AI<R!I<R!A(&=R96%T(&1A>2!T;R!D:64@=&]D87DA")])
        >>> cnbf.brute_force(format="uu_charset", language="english", frequency_required=0.25)
        ...
        >>> cnbf = CaesareNumberBruteForcer([ord(number) for number in list("<0!<D7M∟D7M∟<#!8M41 O#!,<9-∟O6M∟?51/;8!X?32?<")])
        >>> cnbf.brute_force(format="uu_charset", language="english", frequency_required=0.75)
        ...
        if format == "uu_charset":
            ...
            for i in range(30):
                brute_forced_text = caesar.encrypt(self.cipher_text, i)
